...
уведомления о приватных сообщениях.
 ...
 Работа сервера не предусматривает обработку события, когда программа-клиент
 аварийно завершается при подключенном соединении. При этом на сервере хранится
 не удаленный дескриптор, выданный пользователю. Это решается путем удаления старого
 дескриптора, выданного пользователю, при повторной авторизации клиентом. Подозреваю
 что это обрабатывается периодической отправкой пакетов клиенту для проверки соединения.
 В целом, кроме этого сценария, сломать свой сервер из клиента я не смог.
 Все запросы обрабатываются. И верные и не верные.
